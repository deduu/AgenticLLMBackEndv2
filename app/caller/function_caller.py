...
class FunctionCaller:
    ...
    async def execute(
        ...
        return await self.process_user_query(messages = messages, tools= self.tools)
    ...
    # async def _generate_response(self, messages: List[Dict[str, str]], max_new_tokens: int = 128) -> str:
    #     """
    ...
    #     """
    #     logger.info(f"Generating response with messages: {messages}")
    #     inputs = self.llm.tokenizer.apply_chat_template(
    #         messages, 
    ...
    #     response = self.llm.tokenizer.decode(out[0][input_length:], skip_special_tokens=True)
