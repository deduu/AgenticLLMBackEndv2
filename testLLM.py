...
def get_assistant_content(outputs):
    """Extracts the content from the assistant's response."""
    if not isinstance(outputs, list) or not outputs:
        return "Invalid output format"

    first_output = outputs[0]  # Assuming only one output is generated

    if isinstance(first_output, dict) and "generated_text" in first_output:
        last_turn = first_output["generated_text"]
    else:
        return "Invalid structure in output"

    # Debugging
    print("Extracted generated_text:", last_turn)

    # Check if it's already a list/dict instead of a JSON string
    if isinstance(last_turn, (list, dict)):
        return last_turn  # If it's structured, return it directly

    try:
        decoded_json = json.loads(last_turn)
        if isinstance(decoded_json, list):
            for item in decoded_json:
                if isinstance(item, dict) and item.get("role") == "assistant":
                    return item["content"]
        elif isinstance(decoded_json, dict) and decoded_json.get("role") == "assistant":
            return decoded_json["content"]
        else:
            return last_turn  # If it's not a valid JSON structure, return as is
    except json.JSONDecodeError:
        return last_turn  # Return the whole output if JSON parsing fails



def query_with_cot(context, question):
    """Queries the model with Chain-of-Thought prompting."""

    system_prompt = """You are an expert at reasoning and problem-solving. Given a context and a question, you will reason step by step to arrive at the answer. Base your answer *solely* on the information provided in the context. If the answer cannot be directly found in the context, explain how you arrived at the answer by making logical inferences from the context. Let's think step by step. Give your answer according to the language of user query."""
    ...
    outputs = pipeline(
        messages,
        max_new_tokens=1024, # Increased max tokens for longer reasoning
    )
    return get_assistant_content(outputs)

# Example usage:
context = """# Indonesia Framework of Petroleum Resources
...
question = "Apakah angka cadangan boleh 0??"



answer = query_with_cot(context, question)
print(answer)

# context = """The Roman Empire was divided into two halves by Emperor Diocletian in 285 AD. The Western Roman Empire eventually fell in 476 AD, while the Eastern Roman Empire, also known as the Byzantine Empire, continued for another thousand years."""
# question = "How long did the Eastern Roman Empire last after the division of the Roman Empire?"
# answer = query_with_cot(context, question)
# print(answer)

# context = "There are 3 houses. The first house is blue. The second house is red. The third house is yellow."
# question = "What color is the third house?"
# answer = query_with_cot(context, question)
# print(answer)
