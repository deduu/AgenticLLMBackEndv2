...
# --------------------- Query Functions --------------------- #
# async def trends_in_value(db: AsyncSession, start_year: int, end_year: int) -> List[Dict[str, Any]]:
#     stmt = (
#         select(EconomicSectorData.sector, EconomicSectorData.year, func.sum(EconomicSectorData.value).label("total_value"))
#         .where(EconomicSectorData.year.between(start_year, end_year))
#         .group_by(EconomicSectorData.sector, EconomicSectorData.year)
#         .order_by(EconomicSectorData.sector, EconomicSectorData.year)
#     )
#     results = await db.execute(stmt)
#     return results.mappings().all()

async def trends_in_value(
    db: AsyncSession, 
    start_year: int, 
    end_year: int, 
...
# async def total_value_by_country(db: AsyncSession, year: int) -> List[Dict[str, Any]]:
#     stmt = (
...
async def total_value_by_sector(db: AsyncSession) -> List[Dict[str, Any]]:
    stmt = (
...
async def main():
    ...
    async with session_manager.create_session() as db:
        ...
        # Example queries
        trends = await trends_in_value(db, start_year=2010, end_year=2024, countries=["Singapore", "Malaysia", "Brunei Darussalam", "Cambodia", "Laos", "Myanmar", "Thailand", "Philippines", "Vietnam"])
        logger.info(f"Trends in value: {trends}")

        total_value = await total_fdi_value_by_country(db, 2020, 2024)
        logger.info(f"Total FDI value by country: {total_value}")

        sector_totals = await  total_value_by_sector(db)
        logger.info(f"Total sector value: {sector_totals}")
